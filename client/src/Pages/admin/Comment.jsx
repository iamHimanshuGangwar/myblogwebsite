...
const Comment = () => {
  ...
  const tabs = [
    { label: 'Not Approved', icon: <Clock size={16} />, count: pendingCount },
    { label: 'Approved', icon: <CheckCircle2 size={16} />, count: approvedCount },
  ];
    ...
    <div className="flex-1 min-h-screen bg-gray-50 dark:bg-gray-900 p-4 sm:p-8 transition-colors duration-300">
      ...
      <div className="flex p-1 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 w-full sm:w-fit mb-6 shadow-sm">
        {tabs.map(tab => (
          ...
          >
            {tab.icon} {tab.label === 'Approved' ? 'Approved' : 'Pending'}
            <span
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 text-center px-4">
              ...
              <p className="text-gray-500 dark:text-gray-400 mt-1">
                {filter === 'Not Approved'
                  ? "You're all caught up! No pending moderation."
                  : 'No approved comments found yet.'}
              </p>
            </div>
          )}
        </div>
